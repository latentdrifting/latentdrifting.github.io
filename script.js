function copyBibTeX() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (imageCarousel) {
      new Carousel(imageCarousel, 3000);
    }
    
    if (videoCarousel) {
      new Carousel(videoCarousel, 5000);
    }

    // Add touch support
    const carouselElements = [imageCarousel, videoCarousel].filter(el => el);
    carouselElements.forEach((container) => {
      const carousel = container.__proto__.carousel;
      let touchStartX = 0;
      let touchEndX = 0;
